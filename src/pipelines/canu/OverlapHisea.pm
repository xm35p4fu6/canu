...
#  Map long reads to long reads with hisea.
...
sub hiseaConfigure ($$$) {
    ...
    my $base;                #  e.g., $base/1-overlapper/hisea.sh
    my $path;                #  e.g., $path/hisea.sh
    ...
    if (fileExists("$path/queries.tar")) {
        print STDERR "--\n";
        print STDERR "-- OVERLAPPER (hisea) (correction) complete, not rewriting scripts.\n"  if ($tag eq "cor");
        print STDERR "-- OVERLAPPER (hisea) (trimming) complete, not rewriting scripts.\n"    if ($tag eq "obt");
        print STDERR "-- OVERLAPPER (hisea) (assembly) complete, not rewriting scripts.\n"    if ($tag eq "utg");
        print STDERR "--\n";
    ...
    #    my $javaMemory    = int(getGlobal("${tag}hiseaMemory") * 1024 * 0.9 + 0.5);
    ...
    print F "  -noreadname \\\n";
    print F "  -fasta \\\n";
    print F "  -o ./blocks/\$job \\\n";
    print F "|| \\\n";
    print F "mv -f ./blocks/\$job.fasta ./blocks/\$job.fasta.FAILED\n";
    print F "\n";
    ...
    print F "  --threads 40  \\\n";
    print F "  --ignore ../../0-mercounts/$asm.ms$merSize.ignore.gz", "\\\n"   if (-e "$base/0-mercounts/$asm.ms$merSize.ignore.gz");
    #    print F "  --ref $path/blocks/\$job.fasta", "\\\n";
    print F "  --ref ./\$job.fasta", "\\\n";
    print F "  --index_write .",  "\\\n";
    print F "|| \\\n";
    print F "mv -f ./\$job.dat ./\$job.dat.FAILED\n";
    print F "\n";
    ...
    my $hiseaPipe = getGlobal("${tag}hiseaPipe");

    print F "outPath=\"$stageDir\"\n"          if ( defined($stageDir) && ($hiseaPipe == 0));
    ...
    print F "echo \"\"\n";
    print F "\n";
    print F "if [ ! -e ./results/\$qry.hisea.ovb ] ; then\n";

    if ($hiseaPipe) {
        print F "  #  Make a fifo so we can check return status on both\n";
        print F "  #  hisea and hiseaConvert, and still pipe results so we\n";
        ...
        print F "    -S ../../$asm.seqStore \\\n";
        print F "    -o ./results/\$qry.hisea.ovb.WORKING \\\n";
    ...
    print F "    --ref ./blocks/\$blk.fasta \$slf", "\\\n";
    print F "    --index_read ./blocks",  "\\\n";
    #    print F "    --query $path/queries/\$qry", "\\\n";
    print F "    --query ./queries/\$qry", "\\\n";

    if ($hiseaPipe) {
        print F "  > \$qry-pipe \\\n";
        print F "  && \\\n";
        print F "  touch ./results/\$qry.hisea.success\n";
        print F "\n";
        print F "  #  Wait for hiseaConvert to finish.\n";
        print F "  wait\n";
        print F "\n";
        print F "  #  Now that they're done, check status.\n";
        print F "  if [ -e ./results/\$qry.hisea.success -a -e ./results/\$qry.mcvt.success ] ; then\n";
        print F "    mv ./results/\$qry.hisea.ovb.WORKING ./results/\$qry.hisea.ovb\n";
        print F "    rm -f ./results/\$qry.hisea.success\n";
        ...
        print F "  > \$outPath/\$qry.hisea.WORKING \\\n";
        print F "  && \\\n";
        print F "  mv -f \$outPath/\$qry.hisea.WORKING \$outPath/\$qry.hisea\n";
        print F "fi\n";
        print F "\n";
        print F "if [   -e \$outPath/\$qry.hisea -a \\\n";
        print F "     ! -e ./results/\$qry.ovb ] ; then\n";
        print F "  \$bin/hiseaConvert \\\n";
        print F "    -S ../../$asm.seqStore \\\n";
        print F "    -o ./results/\$qry.hisea.ovb.WORKING \\\n";
        print F "    -minlength ", getGlobal("minOverlapLength"), " \\\n";
        print F "    \$outPath/\$qry.hisea \\\n";
        print F "  && \\\n";
        print F "  mv ./results/\$qry.hisea.ovb.WORKING ./results/\$qry.hisea.ovb\n";
        print F "fi\n";
        print F "\n";
        ...
        if (getGlobal('purgeOverlaps') ne "never") {
            print F "if [   -e \$outPath/\$qry.hisea -a \\\n";
            print F "       -e ./results/\$qry.hisea.ovb ] ; then\n";
            print F "  rm -f \$outPath/\$qry.hisea\n";
            print F "fi\n";
            print F "\n";

            #     print F "Ok  Clean up, remove the fasta input\n";
            #     print F "rm -f ./\$blk.fasta\n";
            #     print F "\n";
        }
    }
    ...
    if (getGlobal("${tag}ReAlign") eq "1") {
        ...
        print F "    -S ../../$asm.seqStore \\\n";
        print F "    -O ./results/\$qry.hisea.ovb \\\n";
        ...
        print F "if [ -e ./results/\$qry.hisea.ovb ] ; then\n";
        print F "  mv -f ./results/\$qry.hisea.ovb ./results/\$qry.ovb\n";
        print F "fi\n";
    }
    ...
    makeExecutable("$path/hisea.sh");

    stashFile("$path/precompute.sh");
    stashFile("$path/hisea.sh");
    stashFile("$path/queries.tar");

  finishStage:
    generateReport($asm);
    resetIteration("$tag-hiseaConfigure");
...
sub hiseaPrecomputeCheck ($$$) {
    ...
    goto allDone   if (fileExists("$path/hisea.files"));
    goto allDone   if ((-d "$base/$asm.ovlStore") || (fileExists("$base/$asm.ovlStore.tar.gz")));
    ...
    if (scalar(@failedJobs) > 0) {
        ...
        if ($attempt >= getGlobal("canuIterationMax")) {
            print STDERR "--\n";
            print STDERR "-- Hisea precompute jobs failed, tried $attempt times, giving up.\n";
        ...
        if ($attempt > 0) {
            print STDERR "--\n";
            print STDERR "-- Hisea precompute jobs failed, retry.\n";
            print STDERR $failureMessage;
            print STDERR "--\n";
        }
        ...
        submitOrRunParallelJob($asm, "${tag}hisea", $path, "precompute", @failedJobs);
        return;
    }
  ...
  finishStage:
    ...
    generateReport($asm);
    resetIteration("$tag-hiseaPrecomputeCheck");
...
sub hiseaCheck ($$$) {
    ...
    while (<F>) {
        if (m/^\s+qry=\"(\d+)\"$/) {
            if      (fileExists("$path/results/$1.ovb.gz")) {
                push @hiseaJobs,    "1-overlapper/results/$1.hisea\n";
    ...
    if (scalar(@failedJobs) > 0) {
        ...
        if ($attempt >= getGlobal("canuIterationMax")) {
            print STDERR "--\n";
            print STDERR "-- Hisea overlap jobs failed, tried $attempt times, giving up.\n";
            print STDERR $failureMessage;
            print STDERR "--\n";
            caExit(undef, undef);
        }
        ...
        if ($attempt > 0) {
            print STDERR "--\n";
            print STDERR "-- Hisea overlap jobs failed, retry.\n";
            print STDERR $failureMessage;
            print STDERR "--\n";
        }
        ...
        submitOrRunParallelJob($asm, "${tag}hisea", $path, "hisea", @failedJobs);
        return;
    }
